...
const CocktailList: React.FC = () => {
  ...
  return (
    <div className="flex justify-center items-center w-full">
      <motion.div className="min-h-screen text-white px-6 py-10 max-w-screen-2xl w-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
        {loading ? (
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-full" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.15 } } }}>
            {cocktails.map((cocktail) => {
              return (
                <motion.div key={cocktail.Id} variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0 } }} whileHover={{ scale: 1.05 }} transition={{ type: "spring", stiffness: 200 }} className="min-w-0 flex justify-center">
                  <div className="flex items-end w-full aspect-[4/5] bg-cover rounded-[32px] text-end text-white bg-[0%_70%] shadow-2xl" style={{ backgroundImage: `url(/image_cock/${cocktail.Image})`, boxShadow: "inset 0px -120px 56px -24px rgba(0,0,0,0.63)" }}>
